...
# deltaR is a vector of the binary indicator of belonging to the nonprobability sample;
...
# \item \code{Mu}:  adjusted mean
# \item \code{SE}: adjusted SE
# \item \code{RW}: the associated estimated pseudo weights for units of S_A
# }
...
AIPW_KH <- function(y, x, deltaR, sw, method=c("PAPP", "PMLE", "IPSW", "PM", "IPSW"), spec=c(T, T), family=c('gaussian', 'binomial')){

# Model specification in the simulation  
  	if(sum(spec)==2){
		PS_f <- PM_f <- c("X1", "X2", "X3", "X4")
	}else if(sum(spec)==0){
		PS_f <- PM_f <- c("X1", "X2", "X3")
	}else if(spec[1]==T & spec[2]==F){
		PS_f <- c("X1", "X2", "X3", "X4")
		PM_f <- c("X1", "X2", "X3", "X5")
	}else if(spec[1]==F & spec[2]==T){
		PS_f <- c("X1", "X2", "X3", "X5")
		PM_f <- c("X1", "X2", "X3", "X4")		
	}
	...
	nA <- sum(1-deltaR) # Sample size in S_A
	nR <- sum(deltaR) # Sample size in S_R
	nAR <- nA + nR # Total sample size
	loc.A <- which(deltaR==0) # Indicators of being in S_A
	loc.R <- which(deltaR==1) # Indicators of being in S_R
	...
	y.R <- y.AR[loc.R] # Outcome in S_R
	N <- sum(sw[loc.A]) # Estimated population size
	sw.A <- sw[loc.A] # Sampling weights of S_R in S_A
	sw.B <- sw[loc.R] # Sampling weights of S_R in S_R

	nms <- out_m <- out_se <- pwght <- c() #empty oututs
	
	# Estimate propensity weighted estimate and associated asmptotic SE based on PAPP
	if("PAPP"%in%method){
		fit0 <- glm(z~., data=data.frame(X=x.ARp[,-1], z=deltaR), family=binomial(link="logit"))  # Fit P(Z_i=1|X)
		ei <- exp(predict(fit0, type="link"))
		weight <- sw.B/ei[loc.R]  # Estimate PAPP weights
		est.pdr <- weighted.mean(y.R, w=weight, na.rm=T) # Comput weighted estimates
		# Compute asymptotic SE using TSL method
		wd <- (y.R-est.pdr)*weight
		b <- (t(wd)%*%x.ARp[loc.R, ]/N)%*%ginv(t(x.ARp)%*%(x.ARp*ei/(N*((1+ei)^2))))
		A <- (1/N^2)*sum((1-1/weight)*(wd^2))
		B <- (t(wd/(1+ei[loc.R]))%*%x.ARp[loc.R, ]/(N^2))%*%t(b)
	...
	if("PMLE"%in%method){
		W <- sw
		W[loc.R] <- 1
		PZi <- chen_glm(X=x.ARp[,-1], Y=deltaR, di=W)$predict  #Fit PS model based on Chen et al's PMLE method
		weight <- 1/PZi[loc.R] # Estimate PMLE weights
		# Compute asymptotic SE using TSL method 
		est.pdr <- weighted.mean(y.R, w=weight, na.rm=T)
		wd <- (y.R-est.pdr)*weight
		b <- (t((weight-1)*(y.R-est.pdr))%*%x.ARp[loc.R, ])%*%ginv(t(x.ARp[loc.A, ])%*%(x.ARp[loc.A, ]*sw.A*PZi[loc.A]*(1-PZi[loc.A])))
		D <- nA*cov(PZi[loc.A]*sw.A*x.ARp[loc.A, ])/(N^2)
		#D <- t((sw.A^2 - sw.A) * x.ARp[loc.A, ]) %*% x.ARp[loc.A, ]/(N^2)
		se.pdr <- sqrt((1/N^2)*sum((1-1/weight)*((wd-x.ARp[loc.R, ]%*%t(b))^2)) + b%*%D%*%t(b))
		out_m <- c(out_m, est.pdr)
		out_se <- c(out_se, se.pdr)
		pwght <- cbind(pwght, weight)
		nms <- c(nms, "PMLE")
	...
	if("IPSW"%in%method){
		W <- sw
		W[loc.R] <- 1
		PZi <- wglm(X=x.ARp[,-1], Y=deltaR, di=W)$predict
		weight <- (1-PZi[loc.R])/PZi[loc.R]
		est.pdr <- weighted.mean(y.R, w=weight, na.rm=T)
		wd <- (y.R-est.pdr)*weight
		b <- (t((est.pdr-y.R)/weight)%*%x.ARp[loc.R, ])%*%ginv(t(x.ARp[loc.R, ])%*%(x.ARp[loc.R, ]*PZi[loc.R]))
		D <- nA*cov(PZi[loc.A]*sw.A*x.ARp[loc.A, ])/(N^2)
		#D <- t((sw.A^2 - sw.A) * x.ARp[loc.A, ]) %*% x.ARp[loc.A, ]/(N^2)
		se.pdr <- sqrt((1/N^2)*sum((1-1/weight)*((wd-x.ARp[loc.R, ]%*%t(b))^2)) + b%*%D%*%t(b))
		out_m <- c(out_m, est.pdr)
	...
	if("PM"%in%method){
		if(family=="gaussian"){
			fit1 <- lm(y~., data=data.frame(x.Rm[,-1], y=y.R))
			pred <- predict(fit1, newdata=data.frame(x.Am[,-1]),type="response",se.fit = TRUE)
			m.A <- pred$fit
			est.pdr <- weighted.mean(m.A, w=sw.A, na.rm=T)
			c <- ginv(t(x.ARm[loc.R, ])%*%x.ARm[loc.R, ]/nR)%*%t(t(sw.A)%*%x.ARm[loc.A, ]/N)
			ve.pdr <- weighted.var(y0=m.A, w0=sw.A) + sum((fit1$residuals^2)*(x.ARm[loc.R, ]%*%c)^2)/(nR^2)
			se.pdr<-sqrt(ve.pdr)
		}else if(family=="binomial"){
			fit1 <- glm(y~., data=data.frame(x.Rm[,-1], y=y.R), family=binomial(link="logit"))
			m.A <- predict(fit1, newdata=data.frame(x.Am[,-1]), type="response")
			m.B <- predict(fit1, type="response")
			est.pdr <- weighted.mean(m.A, w=sw[loc.A], na.rm=T)
			c <- ginv(t(x.ARm[loc.R, ])%*%x.ARm[loc.R, ]/nR)%*%t(t(sw.A)%*%x.ARm[loc.A, ]/N)
			ve.pdr <- weighted.var(y0=m.A, w0=sw.A) + sum((fit1$residuals^2)*((x.ARm[loc.R, ]*m.B*(1-m.B))%*%c)^2)/(nR^2)
			se.pdr<-sqrt(ve.pdr)			
		}
		out_m <- c(out_m, est.pdr)
		out_se <- c(out_se, se.pdr)
	...
	if("PAPP"%in%method & "AIPW"%in%method){
		if(family=="gaussian"){
			...
			par0 <- rep(0, (pp + pm + 2))
			par0[1:(pp+1)] <- glm(z~., data=data.frame(X=x.ARp[,-1], z=deltaR))$coefficients
			par0[(pp+2):(pp + pm + 2)] <- lm(y~., data=data.frame(X=x.Rm[,-1], y=y.R))$coefficients
			jeepar<-rootSolve::multiroot(Uee_papp,par0,deltaR=deltaR,x.ARp=x.ARp,x.ARm=x.ARm,y.AR=y.AR,sw=sw)$root
			...
			##################################################
			est.pdr <- sum((y.AR-m.est)*deltaR/piB.est)/N + sum(m.est*(1-deltaR)*sw)/N
			m.A <- m.est[loc.A]
			sigmasqhat <- sum((y.AR[loc.R]-m.est[loc.R])^2)/(nR-pm-1)
			W2 <-   #nA*var(m.est[loc.A]*sw.A)/(N^2)  #sum((sw.A^2 - sw.A) * (m.A)^2)/(N^2)
			ve.pdr <- weighted.var(y0=m.A, w0=sw.A) + (sum(deltaR*(1-2*piB.est )/piB.est ^2) + (N )) * sigmasqhat/(N^2)
			se.pdr<-sqrt(ve.pdr)
			...
			##################################################
			par0 <- rep(0, (pp + pm + 2))
			par0[1:(pp+1)] <- glm(z~., data=data.frame(X=x.ARp[,-1], z=deltaR), family=binomial(link="logit"))$coefficients
			par0[(pp+2):(pp+pm+2)] <- glm(y~., data=data.frame(X=x.Rm[,-1], y=y.R), family=binomial(link="logit"))$coefficients
			jeepar<-rootSolve::multiroot(Uee_binary_papp,par0,deltaR=deltaR,x.ARp=x.ARp,x.ARm=x.ARm,y.AR=y.AR,sw=sw)$root
			jeepar1<-jeepar[ 1:(pp + 1)]
			jeepar2<-jeepar[(pp + 2):(pp + pm + 2)]
			lpi.est <- exp(as.vector(x.ARp %*% as.matrix(jeepar1)))
			piZ.est  <- lpi.est/(1+lpi.est)
			...
			ve.pdr <- weighted.var(y0=m.A, w0=sw.A) + (sum(sigmasqhat*deltaR*(1-2*piB.est )/piB.est ^2) + sum(sigmasqhat[loc.A]/sw.A))/(N^2)
			se.pdr<-sqrt(ve.pdr)
		}
		out_m <- c(out_m, est.pdr)
		out_se <- c(out_se, se.pdr)
		pwght <- cbind(pwght, 1/piB.est[loc.R])
		nms <- c(nms, "AIPW_PAPP")
	}
	if("PMLE"%in%method & "AIPW"%in%method){
		if(family=="gaussian"){
			...
			W <- sw
			W[loc.R] <- 1
			par0[1:(pp+1)] <- chen_glm(X=x.ARp[,-1], Y=deltaR, di=W)$coef
			par0[(pp+2):(pp + pm + 2)] <- lm(y~., data=data.frame(X=x.Rm[,-1], y=y.R))$coefficients
			jeepar<-rootSolve::multiroot(Uee_pmle,par0,deltaR=deltaR,x.ARp=x.ARp,x.ARm=x.ARm,y.AR=y.AR,sw=sw)$root
			jeepar1<-jeepar[ 1:(pp+1)]
			jeepar2<-jeepar[(pp+2):(pp + pm + 2)]
			lpi.est <- exp(as.vector(x.ARp %*% as.matrix(jeepar1)))
			...
			par0[1:(pp+1)] <- chen_glm(X=x.ARp[,-1], Y=deltaR, di=W)$coef
			par0[(pp+2):(pp+pm+2)] <- glm(y~., data=data.frame(X=x.Rm[,-1], y=y.R), family=binomial(link="logit"))$coefficients
			jeepar<-rootSolve::multiroot(Uee_binary_pmle,par0,deltaR=deltaR,x.ARp=x.ARp,x.ARm=x.ARm,y.AR=y.AR,sw=sw)$root
			jeepar1<-jeepar[ 1:(pp + 1)]
			jeepar2<-jeepar[(pp + 2):(pp + pm + 2)]
			lpi.est <- exp(as.vector(x.ARp %*% as.matrix(jeepar1)))
			piB.est  <- lpi.est/(1+lpi.est)
	...
	return(list(Method=nms, Mu=out_m, SE=out_se, RW=pwght))
}
...
Uee_papp <- function(par, deltaR, x.ARp, x.ARm, y.AR, sw){
	pp<-ncol(x.ARp)-1
	pm<-ncol(x.ARm)-1
	nAR <- length(deltaR)
	alpha <- par[1:(pp+1)]
	...
	c(apply(x.ARm*(deltaR/piZ-1)*sw,2,sum), apply(x.ARp*deltaR*(1/piZ-1)*resAB*sw,2,sum))/N
}

# joint score equation for alpha and beta of continuous outcome under PMLE
Uee_pmle <- function(par, deltaR, x.ARp, x.ARm, y.AR, sw){
	pp<-ncol(x.ARp)-1
	pm<-ncol(x.ARm)-1
	nAR <- length(deltaR)
	alpha <- par[1:(pp+1)]
	...
	sw <- as.vector(sw)
	resAB <- as.vector(resAB)
	c(apply(x.ARm*deltaR/piB-x.ARm*deltaA*sw,2,sum), apply(x.ARp*deltaR*(1/piB-1)*resAB,2,sum))/nAR
}

# joint score equation for alpha and beta of binary outcome under PAPP
Uee_binary_papp <- function(par,deltaR,x.ARp,x.ARm,y.AR,sw){
	pp<-ncol(x.ARp)-1
	pm<-ncol(x.ARm)-1
	if(is.null(pp))pp=1
	if(is.null(pm))pm=1
	...
	resAB<-as.vector(resAB)
	mAB<-as.vector(mAB)
	c(apply(x.ARm*mAB*(1-mAB)*(deltaR/piZ-1)*sw,2,sum), apply(x.ARp*deltaR*(1/piZ-1)*resAB*sw, 2, sum))/N
}

# joint score equation for alpha and beta of binary outcome under PMLE
Uee_binary_pmle <- function(par,deltaR,x.ARp,x.ARm,y.AR,sw){
	pp<-ncol(x.ARp)-1
	pm<-ncol(x.ARm)-1
	if(is.null(pp))pp=1
	if(is.null(pm))pm=1
	...
	resAB<-as.vector(resAB)
	mAB<-as.vector(mAB)
	c(apply(x.ARm*deltaR/piB*mAB*(1-mAB) -x.ARm*deltaA*sw*mAB*(1-mAB),2,sum), apply(x.ARp*deltaR*(1/piB-1)*resAB,2,sum))/nAR
}
